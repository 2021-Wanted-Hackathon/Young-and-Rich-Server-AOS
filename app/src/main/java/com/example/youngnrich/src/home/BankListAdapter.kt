...
class BankListAdapter(private val context: Context, var bankArrayList: MutableList<banklist>) :
    ...
    fun addItem(banklist: banklist){
        bankArrayList.add(banklist)
        notifyDataSetChanged()
    }

    inner class ItemViewHolder(private val binding: CashItemBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(banklist: banklist, num: Int, context: Context) {

            binding.cashItemTxt1.text = banklist.tag
            binding.cashItemTxt2.text = "${banklist.cash}원"
        }
    } //ViewHolder는 클래스 내에 View를 저장하는 변수를 만들어 그 안에 데이터를 직접 연결시킬 수 있는 클래스, 디자인 패턴

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHolder {
        val binding = CashItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return ItemViewHolder(binding)
    } //화면을 최초 로딩하여 만들어진 View가 없는 경우 레이아웃을 inflate하여 viewHolder를 생성

    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        holder.bind(bankArrayList[position], position, context)
